...
/**
 * The client half of {@link EventHandlerImpl}, kept in its own class on purpose.
 *
 * <p>Fabric strips {@code @Environment(CLIENT)} members on a dedicated server, but the synthetic lambdas
 * such a method generates are not themselves annotated, so they survive with client types still in their
 * descriptors. Loading the enclosing class server-side then has to resolve those types and fails with
 * {@code Cannot load class ... in environment type SERVER}. Holding them in a client-only class that the
 * server never loads avoids that entirely.
 */
@Environment(EnvType.CLIENT)
public final class EventHandlerClientImpl {
    private EventHandlerClientImpl() {
    }
