...
public interface InteractionEvent {
     ...
     */
    Event<RightClickItem> RIGHT_CLICK_ITEM = EventFactory.createEventResult();
    /**
     ...
     */
    Event<UseItemOnBlock> USE_ITEM_ON_BLOCK = EventFactory.createEventResult();
    /**
     * @see UseBlockWithoutItem#useWithoutItem(Level, Player, BlockState, BlockHitResult)
     */
    Event<UseBlockWithoutItem> USE_BLOCK_WITHOUT_ITEM = EventFactory.createEventResult();
    /**
     * @see UseItemOn#useOn(UseOnContext)
     */
    Event<UseItemOn> USE_ITEM_ON = EventFactory.createEventResult();
    /**
     * @see UseItem#use(Level, Player, InteractionHand)
     */
    Event<UseItem> USE_ITEM = EventFactory.createEventResult();
    /**
    ...
    interface LeftClickBlock {
         ...
         * @param hand   The hand that is used.
         * @param pos    The position of the block in the level. Use {@link Player#level()} to get the level.
         * @param face   The face of the block clicked.
         * @return A {@link EventResult} determining the outcome of the event,
         * the action may be cancelled by the result.
         */
    ...
    interface RightClickItem {
         ...
         * the action may be cancelled by the result.
         */
        EventResult click(Player player, InteractionHand hand);
    }
    ...
    interface FarmlandTrample {
         ...
         * the action may be cancelled by the result.
         */
        EventResult trample(Level world, BlockPos pos, BlockState state, double distance, Entity entity);
    }
    ...
    interface UseItemOnBlock {
         ...
         * @return A {@link EventResult} determining the outcome of the event. Any interrupting result replaces the
         * block's behaviour and its {@link EventResult#asMinecraft()} value is returned to vanilla in its place; build
         * it with {@link EventResult#fromMinecraft(InteractionResult)} to name an exact vanilla outcome.
         */
        EventResult useItemOn(Level level, Player player, InteractionHand hand, ItemStack stack, BlockState state, BlockHitResult hit);
    }
    ...
    interface UseBlockWithoutItem {
         ...
         * it with {@link EventResult#fromMinecraft(InteractionResult)} to name an exact vanilla outcome.
         */
        EventResult useWithoutItem(Level level, Player player, BlockState state, BlockHitResult hit);
    }
    ...
    interface UseItemOn {
         ...
         * @return A {@link EventResult} determining the outcome of the event. Any interrupting result replaces the
         * item's behaviour and its {@link EventResult#asMinecraft()} value is returned to vanilla in its place; build
         * it with {@link EventResult#fromMinecraft(InteractionResult)} to name an exact vanilla outcome.
         */
        EventResult useOn(UseOnContext context);
    }
    ...
    interface UseItem {
         ...
         * it with {@link EventResult#fromMinecraft(InteractionResult)} to name an exact vanilla outcome.
         */
        EventResult use(Level level, Player player, InteractionHand hand);
    }
